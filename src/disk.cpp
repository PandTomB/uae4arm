...
typedef enum { ADF_NONE = -1, ADF_NORMAL, ADF_EXT1, ADF_EXT2, ADF_FDI, ADF_IPF, ADF_PCDOS, ADF_KICK, ADF_SKICK, ADF_NORMAL_HEADER } drive_filetype;
typedef struct {
...
static void drive_write_data (drive * drv)
{
  ...
  if (drv->writediskfile) {
	  drive_write_ext2 (drv->bigmfmbuf, drv->writediskfile, &drv->writetrackdata[tr], drv->tracklen);
  }
  switch (drv->filetype) {
  case ADF_NORMAL:
